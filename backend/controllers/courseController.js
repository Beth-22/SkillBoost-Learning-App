...
const createCourse = async (req, res) => {
  try {
    console.log("Received createCourse request:", req.body);
...
const getUserCourses = async (req, res) => {
  try {
    ...
    console.log(`Fetched ${courses.length} courses: ${JSON.stringify(courses.map(c => ({ title: c.title, id: c._id, instructor: c.instructor })))}`);
    res.status(200).json(courses);
  } catch (err) {
